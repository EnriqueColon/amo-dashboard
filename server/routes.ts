...
export async function registerRoutes(httpServer: Server, app: Express) {
  ...
  app.get('/api/stats', (req, res) => {
    ...
    //
    // The states are split by WHAT THE OPERATOR HAS TO DO about them, which is
    ...
    //     malfunction, and calling it red would be crying wolf every day until
    //     a Space exists.
    // Deliberately NOT keyed on "has there ever been a successful run", which
    // was the first cut: it made a perfectly healthy job that simply lacks
    // credentials indistinguishable from one that is failing outright.
    const lastBk = stmts.lastBackup.get() as any;
    const goodBk = stmts.lastGoodBackup.get() as any;
    const lastRunHours = lastBk?.hours_since ?? null;
    const backup_health = {
  ...
  app.get('/api/credit-facility-events/facilities', (req, res) => {
    ...
    // Grouping query (relationship pairs, corporate-family aware) lives in
    // server/lending/facilities.ts — shared with the weekly emailed report.
    const { total, totalFilings, rows } = queryGroupedFacilities(db, {
      county, lender, borrower, facilityType: facility_type,
      startDate: start_date, endDate: end_date, sort, dir,
      limit: limitNum, offset,
    });

    const payload = { total, total_filings: totalFilings, page: pageNum, limit: limitNum, pages: Math.ceil(total / limitNum), rows };
    setCached(cacheKey, payload);
    res.json(payload);
  });
  ...
  // Deal Intelligence was retired on 2026-08-11. The page was unrouted and
  // unreachable by users, and its eight endpoints were consumed by nothing
  // else, so both sides were removed together — deleting the page alone would
  // have left orphaned endpoints still requiring county-correctness forever.
